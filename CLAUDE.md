...
- **Two palettes coexist mid-migration.** The header/footer chrome, the shared `ButtonLink`/`Overline` primitives and the whole home page (`HomeHero`, `JoinBanner`, `HomeDates`, `ChoirBanner`, `PartnersBanner`) use charte tokens; the remaining `app/` pages plus `Concert`, `Article`, `Carrousel`, `ContactForm` still use the legacy `sky-700`/`zinc`/`gray` palette and `container mx-auto`. This is expected mid-refonte, not a bug — **write new work with charte tokens**.
- **Reuse the two shared primitives before styling by hand.** `ButtonLink` is the charter CTA (one shape, four tones: `onLight`, `onDark`, `onTeal`, `outline`); `Overline` is the mono 12px uppercase label. Both are documented in `docs/CHARTE.md`.
- `Carrousel` (+ `assets/contents/carrousel.ts`) is no longer mounted on any page since the M2 home page landed: it is kept on purpose for the upcoming photo gallery page, not dead code to delete.

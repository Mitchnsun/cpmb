...
- `ContactForm` has no backend: it validates client-side and opens a `mailto:` link, so its confirmation says the draft is ready, not that a message was sent. The anti-spam (hidden trap field + a render timestamp rejecting a submission under 2s) is client-side too, and therefore advisory. CPMB-15 also asked for server-side validation and real delivery to `bureau@…`; that was deliberately deferred — see the follow-up issue. There are no env vars or secrets in this project.
